  ...
  (cond [(and (list? e) (not (empty? e)))
         (if (set-member? macros (first e))
             ...
             ;
                 ...
                 ;[(lambda:) 
                 ;   (format "[] (~a) {\n~a;}\n" 
                 ;      (let [(args (second e))] (string-join (for/list ([i (in-range 0 (length args) 2)]) 
                 ;       (format "~a ~a" (list-ref args i) (list-ref args (add1 i)))) ",")) 
                 ;      (string-join (map compile-expr (drop e 2)) ";\n"))]
...
(when (if (equal? (system-type) 'macosx)
          (system (format "clang++ -std=c++1y -pedantic -Wno-parentheses-equality -O3 -o ~a ~a" outbin outsrc))
          (system (format "clang++ -std=c++1y -pedantic -Wno-parentheses-equality -O3 -s -static -o ~a ~a" outbin outsrc)))
  (when (arg-verbose)
